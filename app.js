...
const db = {
    ...
    'btn-silver': { kanji: "銀", color: "#e2e8f0", shadow: "0 0 20px rgba(226, 232, 240, 0.4)", phrase: "銀 (Gin) — Серебро", code: "\n> init('銀');\n> freq: 587.33 Hz [ ⌁ saw ]\n> cond: high", freq: 587.33, wave: 'sawtooth', duration: 1.2, vol: 0.3 },
    'btn-bronze': { kanji: "銅", color: "#ff8c69", shadow: "0 0 20px rgba(255, 140, 105, 0.4)", phrase: "銅 (Dou) — Медь", code: "\n> init('銅');\n> freq: 329.63 Hz [ ⌁ saw ]\n> reso: deep", freq: 329.63, wave: 'sawtooth', duration: 1.8, vol: 0.4 },
...
let typeInterval = null, activeKey = null, keyReleaseTimer = null; 
...
function playElement(keyId) {
    ...
    if (activeKey === keyId) {
        btn.classList.remove('pressed'); hologram.classList.remove('active');
        clearInterval(typeInterval); setTimeout(() => { typewriterDisplay.textContent = ''; }, 800);
        activeKey = null; return;
    }

    btn.classList.add('pressed'); activeKey = keyId;
    keyReleaseTimer = setTimeout(() => { btn.classList.remove('pressed'); activeKey = null; }, 300); 

    playTone(data.freq, data.wave, data.duration, data.vol);

    kanjiDisplay.textContent = data.kanji; kanjiDisplay.style.color = data.color; kanjiDisplay.style.textShadow = data.shadow;
    typewriterDisplay.style.color = data.color; typewriterDisplay.style.textShadow = data.shadow; typewriterDisplay.style.borderLeftColor = data.color;
    hologram.classList.add('active');
    
    clearInterval(typeInterval); typewriterDisplay.textContent = '';
    let i = 0; const fullText = data.phrase + data.code;
    typeInterval = setInterval(() => {
        if (i < fullText.length) { typewriterDisplay.textContent += fullText.charAt(i); i++; } 
        else { clearInterval(typeInterval); }
    }, 40);
}

document.querySelectorAll('.synth-key').forEach(btn => { btn.addEventListener('mousedown', () => playElement(btn.id)); });
document.body.addEventListener('click', initAudio, { once: true });


// --- СИСТЕМНАЯ ПАМЯТЬ (LocalStorage Save/Load) ---
const faders = ['volume', 'pan', 'filter', 'q', 'reverb', 'lfo-spd', 'lfo-dep'];

document.getElementById('btn-save').addEventListener('click', () => {
    let preset = {};
    faders.forEach(fader => {
        preset[fader] = document.getElementById(`fader-${fader}`).value;
    });
    localStorage.setItem('cyberjazz_preset', JSON.stringify(preset));
});

document.getElementById('btn-load').addEventListener('click', () => {
    const preset = JSON.parse(localStorage.getItem('cyberjazz_preset'));
    if (!preset) return;

    faders.forEach(fader => {
        const slider = document.getElementById(`fader-${fader}`);
        slider.value = preset[fader];
        slider.dispatchEvent(new Event('input')); // Обновляем звук аппаратно
    });
});
